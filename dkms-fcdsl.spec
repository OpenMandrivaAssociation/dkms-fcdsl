...
%define card "AVM Audiovisuelles|Fritz DSL ISDN/DSL Adapter"

Summary: dkms package for %{module} driver
Name: dkms-%{module}
Version: %{version}
Release: %mkrel 6
Source0: ftp://ftp.avm.de/cardware/fritzcrd.dsl/linux/suse.93/fcdsl-suse93-3.11-07.tar.bz2
Source1: dkms-fcdsl-use-autoconf-header.patch
Source2: dkms-fcdsl-use-pci_register_driver.patch
Source3: dkms-fcdsl-remove-DECLARE_MUTEX_LOCKED.patch
Source4: dkms-fcdsl-update-irq-flags.patch
Source5: dkms-fcdsl-update-device_interrupt-definition.patch
...
CLEAN="make clean"
AUTOINSTALL="yes"
PATCH[0]="dkms-fcdsl-use-autoconf-header.patch"
PATCH_MATCH[0]="^2\.6\.(19)|([2-9][0-9]+)|([1-9][0-9][0-9]+)"
PATCH[1]="dkms-fcdsl-use-pci_register_driver.patch"
PATCH_MATCH[1]="^2\.6\.(2[2-9])|([3-9][0-9]+)|([1-9][0-9][0-9]+)"
PATCH[2]="dkms-fcdsl-remove-DECLARE_MUTEX_LOCKED.patch"
PATCH_MATCH[2]="^2\.6\.(2[4-9])|([3-9][0-9]+)|([1-9][0-9][0-9]+)"
PATCH[3]="dkms-fcdsl-update-irq-flags.patch"
PATCH_MATCH[3]="^2\.6\.(2[4-9])|([3-9][0-9]+)|([1-9][0-9][0-9]+)"
PATCH[4]="dkms-fcdsl-update-device_interrupt-definition.patch"
PATCH_MATCH[4]="^2\.6\.(19)|([2-9][0-9]+)|([1-9][0-9][0-9]+)"
EOF

tar c . | tar x -C $RPM_BUILD_ROOT/usr/src/%module-%version-%release/

for p in %{_sourcedir}/dkms-fcdsl-use-autoconf-header.patch \
         %{_sourcedir}/dkms-fcdsl-use-pci_register_driver.patch \
         %{_sourcedir}/dkms-fcdsl-remove-DECLARE_MUTEX_LOCKED.patch \
         %{_sourcedir}/dkms-fcdsl-update-irq-flags.patch \
         %{_sourcedir}/dkms-fcdsl-update-device_interrupt-definition.patch;
do
